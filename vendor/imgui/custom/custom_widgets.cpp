...
#include <string>
	...
	bool Keybind(const char* label, int* key, int* mode)
	{
		...
		it_anim->second.text = ImLerp(it_anim->second.text, it_anim->second.active ? c::text::label::active : hovered ? c::text::label::hovered : c::text::label::default_color, GetAnimSpeed());

		it_anim->second.background = ImLerp(it_anim->second.background, g.ActiveId == id ? main_color : c::background_color, GetAnimSpeed());
		it_anim->second.icon = ImLerp(it_anim->second.icon, g.ActiveId == id ? c::main_color : hovered ? c::text::label::hovered : c::text::label::default_color, GetAnimSpeed());
	...
	bool MiniBind(const char* label, int* key, int* mode)
	{
		...
		it_anim->second.background = ImLerp(it_anim->second.background, g.ActiveId == id ? c::main_color : c::background_color, GetAnimSpeed());
		it_anim->second.icon = ImLerp(it_anim->second.icon, g.ActiveId == id ? c::main_color : hovered ? c::text::label::hovered : c::text::label::default_color, GetAnimSpeed());
	...
	bool ChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool cap, ImGuiWindowFlags flags)
	{
		...
		ImVec2 size = ImFloor(size_arg) + ImVec2(0, cap ? 40 : 0);
		...
		SetNextWindowPos(ImVec2(parent_window->DC.CursorPos + ImVec2(0, cap ? 40 : 0)));
		SetNextWindowSize(size - ImVec2(0, cap ? 40 : 0));

		const char* label_end = FindRenderedTextEnd(name);

		GetWindowDrawList()->AddRectFilled(parent_window->DC.CursorPos, parent_window->DC.CursorPos + size, GetColorU32(c::child::background), c::child::rounding, ImDrawFlags_RoundCornersAll);
		...
		GetWindowDrawList()->AddText(parent_window->DC.CursorPos + ImVec2(GetStyle().WindowPadding.x, 25 - CalcTextSize(name).y / 2), utils::GetColorWithAlpha(c::label::default_color, ImGui::GetStyle().Alpha), name, label_end);
		const int vtx_idx_1 = GetWindowDrawList()->VtxBuffer.Size;
		ShadeVertsLinearColorGradientKeepAlpha(GetWindowDrawList(), vtx_idx_0, vtx_idx_1, parent_window->DC.CursorPos + ImVec2(GetStyle().WindowPadding.x, 25 - CalcTextSize(name).y / 2), parent_window->DC.CursorPos + ImVec2(GetStyle().WindowPadding.x, 25 - CalcTextSize(name).y / 2) + CalcTextSize(name), c::main_color, c::main_color);
	...
	bool ThemeButton(const char* id_theme, bool dark, const ImVec2& size_arg)
	{
		...
		PushFont(font::icomoon_page);
		GetWindowDrawList()->AddText(ImVec2(bb.Min.x + (size_arg.x - CalcTextSize("k").x) / 2, bb.Max.y - CalcTextSize("k").y - (size.y - CalcTextSize("k").y) / 2 + it_anim->second.smooth_swap), GetColorU32(c::main_color.Value), "k");
		GetWindowDrawList()->AddText(ImVec2(bb.Min.x + (size_arg.x - CalcTextSize("a").x) / 2, bb.Max.y - CalcTextSize("a").y - (size.y - CalcTextSize("a").y) / 2 - 25 + it_anim->second.smooth_swap), GetColorU32(c::main_color.Value), "a");
		PopFont();
	...
	bool Tab(const char* label, int* v, int number)
	{
		...
		window->DrawList->AddCircleFilled(ImVec2(total_bb.Min.x + 6.f, total_bb.GetCenter().y), it_anim->second.text_offset / 5, GetColorU32(it_anim->second.frame_col));
		window->DrawList->AddText(total_bb.Min + ImVec2(it_anim->second.text_offset, 0.f), GetColorU32(it_anim->second.text_col), label);
	...
	bool Checkbox(const char* label, bool* v)
	{
		...
		ImRect total_bb(pos, pos + ImVec2(w, 19));
		...
		it_anim->second.rect_color = ImLerp(it_anim->second.rect_color, *v ? c::main_color : c::background_color, GetAnimSpeed());
		it_anim->second.text_color = ImLerp(it_anim->second.text_color, *v ? c::text::label::active : hovered ? c::text::label::hovered : c::text::label::default_color, GetAnimSpeed());
		it_anim->second.check_alpha = ImLerp(it_anim->second.check_alpha, *v ? 1.f : 0.f, GetAnimSpeed());
		it_anim->second.rotation = ImLerp(it_anim->second.rotation, *v ? IM_PI / 2 : -IM_PI, GetAnimSpeed());

		GetWindowDrawList()->AddText(ImVec2(total_bb.Min.x, utils::center_text(total_bb.Min, total_bb.Max, label).y),
			GetColorU32(it_anim->second.text_color), label);

		ImRect check_bb(total_bb.Max - ImVec2(total_bb.GetSize().y, total_bb.GetSize().y), total_bb.Max);

		GetWindowDrawList()->AddRectFilled(check_bb.Min, check_bb.Max,
			c::background_color, style.FrameRounding);

		ImRotateStart();
		int vtx_idx_0 = GetWindowDrawList()->VtxBuffer.Size;
		GetWindowDrawList()->AddRectFilled(check_bb.Min, check_bb.Max,
			GetColorU32(ImVec4(1.f, 1.f, 1.f, it_anim->second.check_alpha)), style.FrameRounding);
		int vtx_idx_1 = GetWindowDrawList()->VtxBuffer.Size;
		ShadeVertsLinearColorGradientKeepAlpha(GetWindowDrawList(), vtx_idx_0, vtx_idx_1,
			check_bb.Min, check_bb.Max, c::second_color, c::main_color);

		
		RenderCheckMark(GetWindowDrawList(), check_bb.GetCenter() - ImVec2(5, 5),
			GetColorU32(ImVec4(1.f, 1.f, 1.f, it_anim->second.checkbox_alpha)), 10.f);
		ImRotateEnd(it_anim->second.rotation, check_bb.GetCenter());
	...
	static std::map<ImGuiID, begin_state> combo_states;
	static ImGuiID current_combo_id = 0;

	static void CloseCombo(ImGuiID id)
	{
		auto state = combo_states.find(id);
		if (state == combo_states.end())
			return;

		state->second.opened_combo = false;
		state->second.hovered = false;
		state->second.combo_size = 0.f;
		if (GImGui != nullptr && GImGui->ActiveId == id)
			ClearActiveID();
	}
	...
	bool BeginCombo(const char* label, const char* preview_value, int val, bool multi, ImGuiComboFlags flags)
	{
		...
		auto& state = combo_states[id];
		const int visible_items = ImClamp(val, 1, 6);
		const float opened_size = static_cast<float>((visible_items * 32) + 22);
		const ImRect popup_bb(
			ImVec2(bb.Min.x, bb.Max.y + 5.f),
			ImVec2(bb.Max.x, bb.Max.y + 5.f + opened_size));
		const bool popup_hovered = IsMouseHoveringRect(
			popup_bb.Min,
			popup_bb.Max,
			false);
		...
		if (hovered && g.IO.MouseClicked[0])
		{
			...
			state.hovered = false;
			state.combo_size = state.opened_combo
				? opened_size
				: 0.f;
			if (g.ActiveId == id)
				ClearActiveID();
		}
		else if (state.opened_combo &&
			g.IO.MouseClicked[0] &&
			!popup_hovered)
		{
			CloseCombo(id);
		...
		state.text = ImLerp(state.text, state.opened_combo ? c::text::label::active : hovered ? c::text::label::hovered : c::text::label::default_color, GetAnimSpeed());
		state.background = ImLerp(state.background, state.opened_combo ? c::second_color.Value : c::elements::background, g.IO.DeltaTime * 6.f);
		state.combo_size = state.opened_combo ? opened_size : 0.f;

		GetWindowDrawList()->AddRectFilled(bb.Min, bb.Max, utils::GetColorWithAlpha(c::background_color, c::background_color.Value.w * style.Alpha), c::elements::rounding);

		GetWindowDrawList()->AddText(ImVec2(total_bb.Min.x, utils::center_text(total_bb.Min, total_bb.Max, label).y), GetColorU32(state.text), label);
		...
		if (!state.opened_combo)
			return false;

		ImGui::SetNextWindowPos(popup_bb.Min);
		ImGui::SetNextWindowSize(ImVec2(bb.GetWidth(), state.combo_size));
		...
		ImFormatString(
			...
			"##custom_combo_%08X",
			id);
		bool ret = Begin(popup_name, NULL, window_flags);
		ImGuiWindow* popup_window = GetCurrentWindow();
		BringWindowToDisplayFront(popup_window);
	...
	bool Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
	{
		...
		it_anim->second.background = ImLerp(it_anim->second.background, selected ? c::background_color.Value : utils::GetColorWithAlpha(c::background_color, 0.f).Value, GetAnimSpeed());
	...
	bool SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
	{
		...
		ImVec2 pos = window->DC.CursorPos;
		ImRect total_bb(pos, pos + ImVec2(w, 34));
		ImRect slider_bb(total_bb.Min + ImVec2(0, 6 + CalcTextSize(label).y), total_bb.Max);
		ItemSize(total_bb);
		if (!ItemAdd(total_bb, id)) return false;
		...
		if (st.position == 0.f)
		{
			if (data_type == ImGuiDataType_S32) st.position = float(*(int*)p_data);
			else st.position = *(float*)p_data;
		}
		ImRect grab_bb;
		bool value_changed = SliderBehavior(ImRect(slider_bb.Min, ImVec2(slider_bb.Max.x + 8, slider_bb.Max.y)), id, data_type, p_data, p_min, p_max, format, flags, &grab_bb);
		float speed = GetAnimSpeed();

		st.circle_radius = ImLerp(st.circle_radius, held ? 7.f : hovered ? 6.5f : 6.f, speed);
		st.slow = ImLerp(st.slow, grab_bb.Min.x - slider_bb.Min.x, g.IO.DeltaTime * 12.f);
		st.text_color = ImLerp(st.text_color, held ? c::text::label::active : hovered ? c::text::label::hovered : c::text::label::default_color, speed);

		float real_val = (data_type == ImGuiDataType_S32) ? float(*(int*)p_data) : *(float*)p_data;
		st.position = ImLerp(st.position, real_val, g.IO.DeltaTime * 12.f);
		char value_buf[64];

		if (data_type == ImGuiDataType_S32)
			snprintf(value_buf, sizeof(value_buf), format, int(st.position + 0.5f));
		else
			snprintf(value_buf, sizeof(value_buf), format, st.position);
		auto dl = GetWindowDrawList();
		dl->AddRectFilled(slider_bb.Min, slider_bb.Max, utils::GetColorWithAlpha(c::background_color, c::background_color.Value.w * style.Alpha), c::elements::rounding);

		const int vtx_idx_0 = dl->VtxBuffer.Size;
		dl->AddRectFilled(slider_bb.Min, ImVec2(slider_bb.Min.x + st.slow + 6.f, slider_bb.Max.y), utils::GetColorWithAlpha(c::main_color, style.Alpha), c::elements::rounding);
		const int vtx_idx_1 = dl->VtxBuffer.Size;
		ShadeVertsLinearColorGradientKeepAlpha(dl, vtx_idx_0, vtx_idx_1, slider_bb.Min, ImVec2(slider_bb.Min.x + st.slow + 6.f, slider_bb.Max.y), c::second_color, c::main_color);

		dl->AddShadowCircle(ImVec2(slider_bb.Min.x + st.slow + 6.f - slider_bb.GetSize().y * 0.5f, slider_bb.GetCenter().y), st.circle_radius, ImColor(0.f, 0.f, 0.f, 0.85f), 36, ImVec2(0, 0), false, 36);
		dl->AddCircleFilled(ImVec2(slider_bb.Min.x + st.slow + 6.f - slider_bb.GetSize().y * 0.5f, slider_bb.GetCenter().y), st.circle_radius, utils::GetColorWithAlpha(c::label::active, style.Alpha), 36);

		dl->AddText(total_bb.Min, GetColorU32(st.text_color), label);
		dl->AddText(ImVec2(total_bb.Max.x - CalcTextSize(value_buf).x, total_bb.Min.y), GetColorU32(st.text_color), value_buf);
