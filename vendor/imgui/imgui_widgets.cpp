...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    ImRect bb = bb_frame;
    bb.Expand(ImVec2(
        -static_cast<float>(ImClamp((int)((bb_frame.GetWidth() - 2.0f) * 0.5f), 0, 3)),
        -static_cast<float>(ImClamp((int)((bb_frame.GetHeight() - 2.0f) * 0.5f), 0, 3))));
    ...
    float scroll_max = static_cast<float>(ImMax((ImS64)1, size_contents_v - size_avail_v));
    ...
    float& ani = anim[id];
    ani = ImLerp(ani, held ? 1.0f : hovered ? 0.7f : 0.0f, GetAnimSpeed());
    ImU32 col = utils::GetColorWithAlpha(c::main_color, ani);
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ...
                    if (rect.Overlaps(clip_rect))
                        draw_window->DrawList->AddRectFilled(rect.Min, rect.Max, GetColorU32(c::main_color.Value, 0.2f), c::elements::rounding);
                }
